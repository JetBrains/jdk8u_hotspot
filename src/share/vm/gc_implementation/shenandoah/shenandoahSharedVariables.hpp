...
typedef struct ShenandoahSharedFlag {
  ...
  bool is_set() const {
    return OrderAccess::load_acquire((volatile ShenandoahSharedValue*) &value) == SET;
  }
...
typedef struct ShenandoahSharedBitmap {
  ...
  bool is_unset(uint mask) const {
    assert (mask < (sizeof(ShenandoahSharedValue) * CHAR_MAX), "sanity");
    return (OrderAccess::load_acquire((volatile ShenandoahSharedValue*)&value) & (ShenandoahSharedValue) mask) == 0;
  }
...
struct ShenandoahSharedEnumFlag {
  ...
  T get() const {
    return (T)OrderAccess::load_acquire((volatile ShenandoahSharedValue*) &value);
  }
