...
ShenandoahFreeSet::ShenandoahFreeSet(ShenandoahHeapRegionSet* regions, size_t max_regions) :
        _regions(regions),
        _free_bitmap(max_regions, /* in_resource_area = */ false),
        _max(max_regions)
{
  clear_internal();
}
...
void ShenandoahFreeSet::assert_bounds() const {
  ...
  // would suffer.
  assert (_leftmost <= _max, err_msg("leftmost in bounds: "  SIZE_FORMAT " < " SIZE_FORMAT, _leftmost,  _max));
  assert (_rightmost < _max, err_msg("rightmost in bounds: " SIZE_FORMAT " < " SIZE_FORMAT, _rightmost, _max));

  assert (_leftmost == _max || is_free(_leftmost),  err_msg("leftmost region should be free: " SIZE_FORMAT,  _leftmost));
  assert (_rightmost == 0   || is_free(_rightmost), err_msg("rightmost region should be free: " SIZE_FORMAT, _rightmost));

  size_t beg_off = _free_bitmap.get_next_one_offset(0);
  size_t end_off = _free_bitmap.get_next_one_offset(_rightmost + 1);
  assert (beg_off >= _leftmost, err_msg("free regions before the leftmost: " SIZE_FORMAT ", bound " SIZE_FORMAT, beg_off, _leftmost));
  assert (end_off == _max,      err_msg("free regions past the rightmost: " SIZE_FORMAT ", bound " SIZE_FORMAT,  end_off, _rightmost));
}
#endif
